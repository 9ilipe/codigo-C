...
int main() {
    ...
    // Verificação das regras da porta lógica AND
    if (PORTAA == 1 && PORTAB == 1) {
        printf("SEU NIVEL LOGICO SERA ALTO, OU SEJA, SERA IGUAL A 1\n");
    } 
    else if ((PORTAA == 0 && PORTAB == 0) || 
             (PORTAA == 0 && PORTAB == 1) || 
             (PORTAA == 1 && PORTAB == 0)) {
        printf("SEU NIVEL LOGICO SERA BAIXO, OU SEJA, SERA IGUAL A 0\n");
    } 
    else {
